...
fn load_gnark_json_proof(path: &Path) -> Result<Groth16Proof> {
    ...
    if !proof.commitments.is_empty() {
        return Err(Error::UnsupportedProtocol(
            "gnark proof commitments are not supported by Aptos groth16 verifier generation"
                .to_string(),
        ));
    }
    Ok(Groth16Proof {
...
fn ensure_no_vk_commitments(vk: &GnarkVerificationKeyJson) -> Result<()> {
    if !is_empty_or_null(&vk.commitment_keys) || !vk.public_and_commitment_committed.is_empty() {
        return Err(Error::UnsupportedProtocol(
            "gnark commitment keys are not supported by Aptos groth16 verifier generation"
                .to_string(),
        ));
    }
    Ok(())
...
mod tests {
    ...
    fn rejects_oversized_gnark_binary_before_reading_it() {
        let path = std::env::temp_dir().join(format!(
            "export_aptos_verifier_oversized_gnark_{}",
            std::process::id()
        ));
